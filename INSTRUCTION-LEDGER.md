...
### IL-138: ADR-044 Step 7 — реквантизация qwen3:235b Q4_K_M→Q8_0 на evo2 (execution attempt)
- **Источник:** CEO / оператор, 2026-06-08 — исполнение Шага 7 под санкцией IL-135.
- **Шаги/факт:**
  1. Инвентаризация evo2: `qwen3:235b-a22b-banxe` (3161844859cd, 142 GB), `qwen3:235b-a22b` (754a872f1290, 142 GB) — DONE
  2. `ollama pull qwen3:235b-q8_0` → Error: pull model manifest: file does not exist — FAILED (тег Q8_0 отсутствует в реестре)
  3. Ресурсный анализ: Q4 уже 142 GB > 128 GB RAM evo2; Q8_0 крупнее → не помещается в пул evo2 — BLOCKED
- **Статус:** BLOCKED
- **Proof:** вывод `ollama list` (2 модели 142 GB) + `Error: pull model manifest: file does not exist`; constraint evo2=128GB из ADR-044 context.
- **Deviation:** Step 7 как сформулировано (Q4→Q8_0 на evo2) нереализуем при 128GB RAM и отсутствии Q8_0-манифеста.
- **Blocker:** (a) нет Q8_0-манифеста для qwen3:235b; (b) недостаточно RAM на evo2 под Q8_0 (>142GB). Требуется решение оператора: локальная Q8_0-квантизация из gguf ИЛИ пересмотр target-квантизации в ADR-044 (напр. оставить Q4_K_M / выбрать промежуточный квант).

---

## IL-140-CARDS-CLIENT-FACING-LINE-COMPLETE-2026-06-08
- Date: 2026-06-08 CEST
- Phase (GSD): CLOSE — milestone anchor recording the first client-facing capability delivered BEYOND ADR-049's initial 6 masks, proving the ADR-053 extensibility mechanism (the Cards line, C22). Docs-only IL entry; no code/tests/ADR bodies touched by this entry.
- Type: governance-MILESTONE (factory-delivered code, recorded here; this entry itself is append-only governance).
- Status: DONE.
- Scope: BANXE-only.
- Milestone: first client-facing capability beyond the initial 6 ADR-049 masks delivered end-to-end via the factory — the Cards line (C22) — proving the ADR-053 extensibility path (ADR-authorised mask → CONTRACT port → mask-governed client-facing agent, with the pre-existing domain service-agent sitting behind the port as an adapter).
- Line delivered (3 steps, all via the factory):
  * ADR-053 — mask extensibility + domain-agent-as-adapter-behind-port governance boundary + Cards mask C22 (banxe-architecture PR #356, merged 3970165).
  * CardPort CONTRACT — banxe-emi-stack PR #155 (SHA e99614f): services/card_issuing/card_port.py, 7 ops (read_card / read_limits / freeze / block / unfreeze / issue_card / change_limit), PCI masked-PAN-only, existing card_issuing domain left untouched (to sit behind the port as an adapter).
  * CardsAgent — banxe-emi-stack PR #156 (SHA 82e9e99): services/agents/cards_agent.py, 7th L2 client-facing agent; Mixed mask (reads + protective freeze/block/unfreeze AUTO-with-cap; issue_card/change_limit REVIEW + biometric step-up mandatory), AML + PII gate, ADR-049 §D2 chain, exactly one AgentDecisionRecord per action, reuses the shared _lineage helper (DRY / IL-135), R-SEC/PCI no-secret-in-lineage test-proven, card_issuing domain untouched.
- L2 client-facing agents now (7): Payments, FXExchange, Wallet (banxe-payment-core) + KYC, Notification, CRM, Cards (banxe-emi-stack).
- Capability coverage: C1/C2/C3/C4/C5/C6/C9/C10/C22 client-facing-governed. ~20 capabilities remain (C7 analytics, C11 tariff, C18 news, C19–C30, statements, savings) — each extensible via the same ADR-mask → CONTRACT port → client-facing agent path, with existing domain service-agents becoming adapters-behind-port per ADR-053.
- GATED: live operation (AGENT_ROUTING_ENABLED) remains OFF — depends on Terminal-A LLM-orchestration infra per ADR-049 §D6. This milestone records the Cards L2 CODE + governance as COMPLETE, NOT live client exposure; no CONTRACT port is opened to clients by this entry.
- Note on IL number: IL-138 (ADR-044 Step 7 execution attempt, BLOCKED) is on origin/main; IL-139 is RESERVED by ADR-044 amendment-2026-06-08 (commit 1b2cd98), which forward-references "IL-138/IL-139" for the Step 7 Q8_0-cancellation closure — so IL-139 is NOT free. IL-140 is the genuine next free number (append-only, no collision; 0 tree-wide references to IL-140 confirmed). Authored in an isolated git worktree off origin/main because the shared checkout was dirty (a parallel session's INSTRUCTION-LEDGER.md edit + untracked handoff doc; parallel-session-isolation canon Rule 6).
- Refs: ADR-053 (mask extensibility + domain-agent boundary + Cards mask C22 — the governance line this milestone closes in code; IL-137 SPEC/DESIGN); ADR-049 (Intent Layer & Client-Facing Agent Masks — the initial 6 masks this EXTENDS; §D2 chain, §D3 fields, §D4 thresholds + biometric step-up); ADR-046 (AgentDecisionRecord — one per action, every exit path); ADR-047 (cost caps + AUTO/REVIEW/BLOCK bands — Cards mask cost_cap); ADR-048 (intent→process_ref resolution); ADR-045 (Intent-First four-layer model); ADR-016 (PII/AML routing — Cards compliance_gate); ADR-040 (meta-plane — LLM-orchestration substrate L2 live operation is gated on, Terminal A); CardPort (banxe-emi-stack — the boundary object the Cards mask scopes and card_issuing implements behind); pre-existing card_issuing domain (adapter-behind-port, untouched); IL-132 (L2 layer complete — the initial 6 masks' code) + IL-135 (lineage/cost primitives DRY, _lineage reused here) + IL-137 (ADR-053 governance); NEW-PROJECT-PRIORITY-MAP-2026-06-06 (C1–C30 capability catalogue; C22 brought under governance here); .claude/rules/agents.md (HITL bands; ARL AGENT_ROUTING_ENABLED=false precondition; Ruflo mandatory for payment/compliance-classed ops); CLAUDE.md §11 (production-state mutation gate — code-complete, not a production green light) + money rule (Decimal).
