...
**ADR reference:** ADR-018 — ⚠️ **PENDING AUTHORING** (P1 governance gap).
Production systemd unit (`qwen3-235b-master.service`) references "ADR-018 P4.3-Q235" in its
`Description=` field, but no ADR-018 exists in `docs/adr/`. This is a dangling reference.
Factory task to author ADR-018 is queued.
...
| OD-3 | USB4 peer 10.0.0.1: physically identify + document in ADR-018 | Operator | Q3-2026 |
| OD-4 | ADR-018: author + merge (production systemd has dangling reference) | Factory | P1 — immediate |
