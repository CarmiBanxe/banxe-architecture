...
| **LLM02** | Sensitive Information Disclosure | **input/output-validated-by: Guardrails.ai (#104, proposed — `governance/runtime-guardrails-policy.md` §LLM-input validation layer)** PII/secret validators; secrets governance (`ACCESS-AND-SECRETS.md`, gitleaks); redaction canon | ☐ PASS ☐ GAP ☐ N-A |
...
| **LLM05** | Improper Output Handling | **runtime-enforced-by: NeMo-Guardrails output rails (#65, proposed)**; **input/output-validated-by: Guardrails.ai structured-output validators (#104, proposed)** — both in `governance/runtime-guardrails-policy.md`; no-authority canon (ADR-130/127) | ☐ PASS ☐ GAP ☐ N-A |
...
- Runtime complements (both PROPOSED, `governance/runtime-guardrails-policy.md`): **#65 nemo-guardrails-runtime-safety** (LLM01/LLM05 conversational rails); **#104 guardrails-ai-validators** (LLM02/LLM05 structured-output/PII validators). Together = cluster-1 LLM-safety perimeter.
