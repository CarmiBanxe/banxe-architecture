...
# B->A novelty auto-handoff pipeline — factory-watcher v2.2 (PROPOSED, in-repo).
#
# v2.2 changes vs v2.1 (ADR-060-compliant worktree-branch id; fixes 2nd
# battle-run defect — see docs/canon/TERMINAL-B-OPERATING-CANON.md pointer,
# ADR-102 no-restatement):
#   * WORKTREE BRANCH ID renamed watcher-handoff -> watcherhandoff. The
#     ADR-060 validator (mirrored in scripts/bx-session.sh) requires the
#     `<id>` segment to match `[A-Za-z0-9]+` (no hyphens); the previous
#     `agent/factory/watcher-handoff/current` violated it because
#     "watcher-handoff" contains a hyphen. Result seen in 2nd battle run
#     (02:34:43): "bx-session: branch violates ADR-060" -> worktree not
#     created -> hand-off writes deferred. v2.2 fix: id=watcherhandoff ->
#     branch = `agent/factory/watcherhandoff/current` (actor=factory ✓,
#     id=watcherhandoff ∈ [A-Za-z0-9]+ ✓, slug=current ∈ [a-z0-9._-]+ ✓).
#   * v2.1 sync-before-scan, v1.1 strict-NEW filter, v2 fail-open real
#     LiteLLM :4000 scoring, and single-writer idempotency are ALL
#     PRESERVED — this rev ONLY renames the worktree-branch id.
#
...
#
# v2.2: `<id>` = "watcherhandoff" (no hyphen) to satisfy the ADR-060
# pattern mirrored in scripts/bx-session.sh
# (^agent/(central|right|factory|specproj)/[A-Za-z0-9]+/[a-z0-9._-]+$).
# The prior `<id>` = "watcher-handoff" contained a hyphen and violated the
# id class, so bx-session.sh refused the branch and the worktree was never
# created (2nd battle run 2026-07-06 02:34:43).
...
append_roadmap_marker() {
  ...
  ts=$(date -u +'%Y-%m-%dT%H:%M:%SZ')
  {
    printf '\n<!-- novelty-handoff v2.2: %s @ %s (ADR-159 mandatory hand-off) -->\n' \
      "${item}" "${ts}"
...
novelty-watcher:   git commit -m 'feat(pipeline): novelty hand-off batch [factory-watcher v2.2]'
...
novelty-watcher:     --title 'novelty: hand-off batch (watcher v2.2)' \\
novelty-watcher:     --body 'auto-generated by scripts/novelty-watcher.sh v2.2; HITL merge per CLAUDE.md §71'
