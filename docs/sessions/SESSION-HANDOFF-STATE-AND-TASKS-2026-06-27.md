...
- `legacy_otp_adapter → production/{twilio,sendgrid}_otp_adapter` (provider parity). ⚠ **CORRECTED 2026-06-28: NOT a migrate/retire — `LegacyOtpAdapter` is the shared BASE CLASS that Twilio/SendGrid/stubs inherit (LIVE_KEEP); production adapters are PARKED scaffold (creds/route gated, not wired); OTP-delivery ≠ the wired TOTP runtime path** — see the legacy_otp Correction note in `docs/refactor-legacy/EMI-LEGACY-RATIONALIZATION-PASS-1-2026-06-27.md`.
- `legacy_sepa_adapter → production/modulr_sepa_stub` (Modulr live-wiring). ⚠ **CORRECTED 2026-06-28: NOT a migrate-pair — the SEPA `*_sepa_*` family (Legacy/ModulrSepa/Stub) is UNWIRED (0 runtime consumers); the live SEPA rail is `ModulrPaymentAdapter` (`modulr_client.py`, PAYMENT_ADAPTER=modulr). `LegacySepaAdapter` = PARKED reference; `ModulrSepaAdapter` = PARKED scaffold with an ADR-102 overlap vs the live rail; `ModulrSepaStub` = PARKED Wave-C seam** — see the legacy_sepa Correction note in `docs/refactor-legacy/EMI-LEGACY-RATIONALIZATION-PASS-1-2026-06-27.md`.
