...
| `docs/adr/` | ADR-031 .. ADR-NNN (thematic, Phase-aligned) | YAML frontmatter | Phase-specific or thematic ADRs (e.g. Phase 3 cluster: ADR-031..034). |

## Numbering gap (018–030)

The gap between ADR-017 (last in `decisions/`) and ADR-031 (first in `docs/adr/`) is **intentional and reserved** for future entries in `decisions/` so that `docs/adr/` can keep Phase-aligned numbering without future renumbering.

## Adding a new ADR

- New canonical, repo-wide architectural decision → next free number in `decisions/` (currently ADR-018+).
- New thematic Phase-3+ decision → next free number in `docs/adr/` (currently ADR-035+).
...
| ADR-031 | AI execution policy | Accepted | — |
| ADR-032 | GLM-4.5-Air distributed inference | Accepted | — |
| ADR-033 | ufw perimeter posture per host | Accepted | — |
| ADR-034 | Aider/Continue routes | Accepted | — |
