...
| **H — Customer Operations** | H-crm | CRM — customer record, case history, DSAR (data subject access requests) | Spec-Locked — In Progress (IL-521; H-CRM-BUILD-SPEC; customer golden record (links A-kyc/A-kyb + B-emi) + case history (audit trail, references H-support cases) + DSAR fulfilment (GDPR Art.15-22 access/rectification/erasure/portability, 1-month SLA, identity verification before disclosure, HITL on erasure + legal-hold fail-closed); CustomerRecord/CaseHistory/DSARRequest model; reuses CRMProviderPort, integrates I-security PII Proxy/IAM; privacy-by-design purpose-limitation no-profiling/no-surveillance; no KYC/IAM/PII-infra/ticketing reimpl) | P2 | Sprint 12 | CEO |
| **H — Customer Operations** | H-support | Support ticketing, escalation workflows, SLA tracking | Spec-Locked — In Progress (IL-523; H-SUPPORT-BUILD-SPEC; ticket lifecycle (create/route/assign/resolve) + escalation (priority/SLA-breach/vulnerable/fraud-AML) + SLA tracking (first-response/resolution targets config-as-data) + complaints (FCA DISP 8-week SLA + FOS escalation rights, Consumer Duty outcome monitoring); Ticket/Escalation/SLATimer/Complaint model; delegates helpdesk to support stack (Chatwoot/n8n/Ollama) via SupportProviderPort, integrates I-security PII Proxy; OWNS ticket, emits case refs → H-crm CaseHistory; privacy-by-design no marketing/secondary use; no CRM-record/PII-infra/fraud-AML reimpl) | P2 | Sprint 12 | CEO |
...
| **L — Data Platform** | L-bi | BI / dashboards — management reporting, FCA KPI monitoring | Spec-Locked — In Progress (IL-551; L-BI-BUILD-SPEC; BI/dashboard layer: management reporting + FCA KPI monitoring (safeguarding/recon/Consumer Duty/regulatory-deadline KPIs) + config-as-data KPI engine; reads L-lake/ClickHouse + D-fin; read-only analytics no GL/lake/reporting reimpl; dashboard rendering tier operator-pick P1) | P3 | Sprint 13 | CEO |
| **M — Developer Platform** | M-gateway | API Gateway — public REST API, OpenAPI spec, versioning | Spec-Locked — In Progress (IL-525; M-GATEWAY-BUILD-SPEC; developer-platform PRODUCTISATION layer (NOT a second gateway); public OpenAPI/Swagger publication + SDK generation + developer portal/docs + API-key self-service + versioning/deprecation governance + usage analytics/plan-tier hooks; PublishedAPI/SDKArtifact/DeveloperApp/APIKeyGrant model; PUBLISHES THROUGH I-api (IL-508 enforces routing/auth/rate-limit), keys issued by I-security IAM (no secrets stored), plan tiers from B-pricing; no I-api/IAM/routing/auth/rate-limit reimpl) | P2 | Sprint 12 | CTIO |
| **M — Developer Platform** | M-sdk | SDK — Python + JS client libraries | 0% | P3 | Sprint 14 | CTIO |
| **M — Developer Platform** | M-sandbox | Sandbox environment — test accounts, mock payment rails | Spec-Locked — In Progress (IL-526; M-SANDBOX-BUILD-SPEC; consolidates ACCEPTED sandbox ADRs ADR-086 + ADR-096-101/SBOX-1..6; isolated test/sandbox env: sandbox-mode surface + synthetic test accounts + mock C-fps/C-sepa/C-swift rails (mirror PaymentRailPort, no settlement) + demo scenarios + partner pack + session record/replay + portal shell + optional demo-gamification + read-only Risk&Earn analytics; SandboxTenant/TestAccount/MockRailResponse/DemoScenario/Session model; hard production isolation — no real money/rails/ledger/PII; sandbox keys via M-gateway; no production/rail/ledger reimpl) | P2 | Sprint 12 | CTIO |
...
| P2 (Operational) | 9 | L-lake (30%), B-pricing (Spec-Locked, In Progress — IL-512, commercial pricing catalogue: tiers/rules/published fee schedules, config-as-data; rate source for D-fee engine, no fee-engine/billing reimpl), F-fatca (Spec-Locked, In Progress — IL-514, FATCA/CRS/DAC8 tax reporting; classification + reportable-account determination + FATCA/CRS XML, submits via K-gabriel pattern HITL-gated; consumes A-kyc self-cert + B-emi/D-gl; no KYC/GL/submission reimpl, no advice), C-swift (Spec-Locked, In Progress — IL-517, SWIFT international wires MT/MX + correspondent/NOSTRO; shared PaymentRailPort with C-fps/C-sepa, F-aml pre-send + FATF Travel Rule, gpi/UETR; emits D-recon Leg C + posts D-gl; no GL/recon/AML reimpl), E-treasury (Spec-Locked, In Progress — IL-519, treasury management; operationalises ADR-078 read-only ports; liquidity+FX exposure+ALM ladders, TreasuryAgent L2 + ≥£100k CFO HITL, no autonomous trading; consumes D-gl/NOSTRO, feeds D-fin/E-capital; treasury ≠ capital adequacy, no capital/reporting/rail reimpl), H-crm (Spec-Locked, In Progress — IL-521, CRM golden record + case history + DSAR fulfilment (GDPR Art.15-22, 1-month SLA, identity-verify, HITL erasure + legal-hold); reuses CRMProviderPort, consumes A-kyc/A-kyb, references H-support; privacy-by-design no-profiling/no-surveillance; no KYC/IAM/ticketing reimpl), H-support (Spec-Locked, In Progress — IL-523, support ticketing + escalation + SLA tracking + complaints (FCA DISP 8-week + FOS rights, Consumer Duty); delegates helpdesk to Chatwoot/n8n via SupportProviderPort, OWNS ticket lifecycle, emits case refs → H-crm; privacy-by-design no marketing/secondary use; no CRM-record/PII-infra/fraud-AML reimpl), M-gateway (Spec-Locked, In Progress — IL-525, developer-platform productisation: public OpenAPI/SDKs/portal/self-service keys/versioning policy; publishes THROUGH I-api (I-api enforces routing/auth/rate-limit), keys via I-security IAM, tiers from B-pricing; NOT a second gateway, no I-api/IAM reimpl), M-sandbox (Spec-Locked, In Progress — IL-526, isolated sandbox env consolidating ADR-086 + ADR-096-101/SBOX-1..6; synthetic test accounts + mock C-rails (no settlement) + demo scenarios + session replay + partner pack + portal shell; hard production isolation no real money/PII; keys via M-gateway; no production/rail/ledger reimpl) | All others 0% |
