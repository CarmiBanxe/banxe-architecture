...
| GAP-085 | **ss1 GDPR** — ss1 repo (guiyon/ss1) was PUBLIC until 2026-05-13; may be indexed (Google Cache/archive.org). If personal data present: GDPR Art.33 CNIL notification (72-hour clock from 2026-06-27 awareness). Legal must assess immediately. RECLASSIFIED 2026-07-03: ss1 belongs to guiyon (categorically EXCLUDED from Banxe per canon/GAP-090) — outside Banxe regulatory perimeter. Operator decision (sandbox): Banxe factory takes no CNIL action; any legal assessment is handled by Legal OUTSIDE the sandbox, on the guiyon side. GAP-085 is not a Banxe-blocking item. Decision package docs/audit/gap085-cnil-notification-decision-package-2026-07-02.md retained as reference only. | Legal (CNIL) / Operator | IMMEDIATE if data breach | ⚪ OUT-OF-SCOPE (sandbox) |
