# Model Card — qwen3:4b  (DRAFT — pending operator/CRO approval)
...
## 1. Identity
- **Model / alias:** `qwen3:4b`
- **Role in canon:** fast small model for tools / autocomplete.
...
## 2. Provenance
- **Source / weights:** `ollama` model `qwen3:4b` (verified present via `ollama list`).
- **Size:** 2.5 GB (HW-MODEL-UPGRADE-matrix.md).
...
- **Intended use:** tool-use / autocomplete / minimal-footprint utility — no regulated decision authority.
