# Model Card — qwen3.5:latest  (DRAFT — pending operator/CRO approval)
...
## 1. Identity
- **Model / alias:** `qwen3.5:latest`
- **Role in canon:** embedding / quick reasoning utility (9.7B).
...
## 2. Provenance
- **Source / weights:** `ollama` model `qwen3.5:latest` (verified present via `ollama list`).
- **Size:** 6.6 GB (HW-MODEL-UPGRADE-matrix.md).
...
- **Intended use:** embedding / quick-reasoning utility — no regulated decision authority.
