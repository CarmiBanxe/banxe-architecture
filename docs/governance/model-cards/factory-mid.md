# Model Card — factory-mid (alias)  (DRAFT — pending operator/CRO approval)
...
## 1. Identity
- **Model / alias:** `factory-mid (alias)`
- **Role in canon:** LiteLLM alias — multi-file refactor / spec writing route.
...
## 2. Provenance
- **Alias resolution:** **alias → `qwen3:30b-a3b (MoE)`** — factory-mid → qwen3:30b-a3b, evo1+evo2 :11434 LB (factory-routing-map).
- **Source / weights:** the backing ollama model `qwen3:30b-a3b (MoE)` (verified present via `ollama list`). Callers use the alias only — no direct model IDs in caller config.
- **Size:** 18 GB (backing) (HW-MODEL-UPGRADE-matrix.md).
- **Quantization:** not asserted in repo (HW-matrix size-only for qwen3:30b-a3b).
...
- **Intended use:** multi-file refactor, spec writing (RSB/ACG/CAE/EHS/STG/DO/PS) — advisory factory work.
