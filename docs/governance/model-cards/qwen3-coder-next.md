# Model Card — qwen3-coder-next:q4_K_M  (DRAFT — pending operator/CRO approval)
...
## 1. Identity
- **Model / alias:** `qwen3-coder-next:q4_K_M`
- **Role in canon:** code-focused model (largest non-235b).
...
## 2. Provenance
- **Source / weights:** `ollama` model `qwen3-coder-next:q4_K_M` (verified present via `ollama list`).
- **Size:** 51 GB (HW-MODEL-UPGRADE-matrix.md).
- **Quantization:** **q4_K_M** (asserted in model id).
- **Host / route:** evo2 primary, evo1 fallback (HW-matrix / factory-routing-map).
...
- **Intended use:** coding-tuned software-delivery (heavy refactor / code generation) — utility, no regulated authority.
