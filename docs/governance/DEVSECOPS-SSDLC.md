...
**Status:** GOVERNANCE BASELINE (PARTIAL → being established) · **Sprint:** S2 (DevSecOps/SSDLC)
...
**Current maturity:** PARTIAL. CI exists (secrets-scan, ledger/append-only guardians, ADR validation, Mermaid/Markdown checks). **No** SBOM generation, **no** artifact/container signing, **no** documented threat models, **no** asserted SAST/SCA/DAST tool. This document does not by itself add any blocking gate — see §4 (scaffolds are inert `.template` files) and §6 (GAPS register).
...
| **Development** | Prevent secrets/insecure code entering history | Secrets Scan (`ci.yml › secrets-scan`, gitleaks); Guardian INV-05 "No secrets in commits" (factory-canon §INV-05); branch-naming gate (ADR-060) | SAST on every PR | **GAP** — SAST tool AWAITS OPERATOR |
| **Test** | Verify code + dependencies before merge | `quality-gate.sh` (project canon); ledger rebuild check (`ledger-build.yml`) | SCA (dependency CVEs); DAST (runtime) | **GAP** — SCA/DAST tools AWAIT OPERATOR |
| **Deploy** | Produce + sign verifiable artifacts; provenance | Guardian deterministic gates (factory-canon §4.1); operator/MLRO sign-off gates (factory-canon §8) | SBOM (CycloneDX), artifact signing (Cosign), SLSA provenance | **GAP** — SBOM/signing scaffolded inert (§4) |
...
All three carry a `.template` suffix (or live under `docs/`) and are **inert** — they do not run in CI and add no required gate. Activation AWAITS OPERATOR.

- `.github/workflows/sbom.yml.template` — syft → CycloneDX SBOM artifact.
- `.github/workflows/cosign-sign.yml.template` — cosign keyless/OIDC artifact signing (key/identity policy AWAITS OPERATOR).
- `docs/governance/threat-models/THREAT-MODEL-TEMPLATE.md` — STRIDE template (per-service instances AWAITS OPERATOR).
...
- **Canon:** Operator supreme over docs; no facts invented; unknowns = AWAITS OPERATOR. Scaffolds are inert (`.template`) — no required gate added, existing guardians untouched. DO NOT MERGE pending operator review.
