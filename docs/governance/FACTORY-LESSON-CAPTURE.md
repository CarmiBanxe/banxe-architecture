...
| L-10 | A merged fleet audit mis-stated an inventory + a status tally (§1 omitted `agents/swarms/*`; §2 read `41/16` not `39/18`) | An inventory/tally that did **not state its corpus + measurement rule** — §1 enumerated only 3 path-classes (missed one), §2 counted `^\s*status:` (whitespace-tolerant) yet **labelled** it "top-level", folding 2 indented `status:` into the count | **A tally must state its corpus and measurement rule explicitly**: top-level status counts **anchor to `^status:`** (un-indented, case-sensitive); **nested/indented status is reported separately**, never folded into the top-level count; and an **audit label must match its counting method exactly** | #972/IL-816, this erratum |
