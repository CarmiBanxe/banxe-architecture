# AGENT REGISTRY — MASTER (consolidated 4-floor) — 2026-07-22

> **SUPERSEDED for bank-headcount (2026-07-23):** this combined 147-row MASTER mixed agents from multiple companies. For **bank** agents use `AGENT-REGISTRY-BANK-MASTER-2026-07-22.md` (129 rows). Ownership split: `AGENT-OWNERSHIP-SPLIT-2026-07-22.md`. This file is retained as the full pre-split consolidation, not the bank source of truth.

**GOVERNANCE / MASTER AGENT REGISTRY / SOURCE OF TRUTH (CONFIRMED-5) / DOCS-ONLY / READ-ONLY RUNTIME**
Consolidation of the per-floor registries (F1–F4). Source-of-truth per CONFIRMED-5. Verified: **census `*_agent.py` = 86, all placed exactly once (UNPLACED = 0); no duplicate rows**. The per-floor registries remain the editable source; this MASTER is a read-only consolidation (rows collected verbatim, agent_id preserved). Read-only over `~/banxe-emi-stack`.

## Consolidated table (sorted floor → room)
...
- **Total rows:** 147.
- **Census `*_agent.py`:** 86 (all placed exactly once → UNPLACED = 0).
- **Functional non-suffix added (separate agent_id):** 61.
- **Grand total = 86 + 61 = 147.**

**Per-floor:** F1 = 34 · F2 = 48 · F3 = 40 · F4 = 25.

**Per-room:** F1-support 8 · F1-marketing 6 · F1-customer-ops 18 · F1-hr-legal 2 · F2-identity 8 · F2-ledger 9 · F2-payments 27 · F2-safeguarding 4 · F3-aml 10 · F3-risk 10 · F3-treasury 10 · F3-finbi 6 · F3-regrep 4 · F4-ai-platform 7 · F4-devops 9 · F4-security 2 · F4-audit-cell 7.

**Classification:** decision = 111 · tooling = 36.
**Flags:** `[gated-counsel]` = 21 · `[pending human ratification]` = 20.

### Census check

`census *_agent.py = 86` · `functional non-suffix added = 61` · `grand total = 86 + 61 = 147`. Census coverage = **86 / 86 (100%)**; duplicate rows = 0.

## MASTER verdict

- **Registry structurally complete:** 100% of census `*_agent.py` (86/86) placed exactly once across F1–F4; no duplicate rows.
- **20 rows `[pending human ratification]`** — routing/room contested; addressed to `[audit]`. Listed in `AGENT-REGISTRY-PENDING-RATIFICATION-2026-07-22.md`. These do **not** block structure.
- **21 rows `[gated-counsel]`** — carve-out surfaces (crypto/CASP, open-banking AISP/PISP, KYB↔acquiring, Midaz/MCP→ledger, cards register #2 RED, credit_scoring Annex III, psd2). Recorded, **not closed**; addressed to `[counsel]`.
- **Correction logged:** two census agents (`consumer_duty_agent`, `design_pipeline/agents/onboarding_agent`) were previously prose-only cross-floor notes; now rowed (AG-F3-040, AG-F4-025). The prior "UNPLACED=0" held for the 23-list closure but missed these two prose exclusions — now genuinely 0.
- `[factory]` to confirm the counting canon (census 86 vs class 77 reconciliation).
