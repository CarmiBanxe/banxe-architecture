...
- **θ threshold (config-as-data).** The polish-loop taste-score threshold (θ) lives as **config-as-data** (location parallel to the token machine-source, §3.1), **not** in this canon. Its **value = `on-canon`** (band threshold; operator-set 2026-06-30, §8 OI-5) — the impeccable-loop stop-condition treats taste as satisfied at the **on-canon** band; taste remains **advisory, never a gate**. The authoritative runtime config-as-data lands project-side (`banxe-ui`, §3.1) when the loop is built.
...
| OI-5 | Taste-score threshold **θ** for the polish loop (config-as-data location + value) + named **Taste & Polish owner** (Design System Lead / Head of Design) | **θ: `on-canon`** (band threshold, operator-set 2026-06-30) · **owner: INTERIM CTO (SMF26)** | θ = on-canon band (impeccable-loop stop-condition only; taste advisory, never a gate); runtime config-as-data project-side (`banxe-ui` §3.1); owner = interim CTO (§7.2). **Dedicated design-lead hire still AWAITS OPERATOR.** |
