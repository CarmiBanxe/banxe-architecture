# F1/customer-ops-room

## Purpose / coverage
Lifecycle, churn, notifications, preferences; dormant product agents (savings/insurance).

## Key agents/services
`CustomerLifecycleAgent`, `savings_agent` (dormant), `insurance_agent` (dormant), `notification_agent`.

## Regulatory Status Notes
- Register area: **#4 New Products (AMBER)**.
- Canonical source: `../../docs/governance/OPEN-REGULATORY-QUESTIONS-REGISTER-2026-07-20.md`.
- Freeze: "No product shall be treated as migration-ready or governance-resolved (GREEN) without a completed evidence pack and permissions mapping."

### Sprint 3 (New Products)
Artefacts: `../../docs/sprints/sprint-3-product-evidence-pack-template.md` · `sprint-3-per-product-evidence-packs.md` · `sprint-3-permissions-map-per-product.md`. Акцент комнаты: customer outcomes, support, fair-value evidence. Per-product паки заполняются до пилота; связь с #4 — до любого AMBER→GREEN.
